...
def process_file(file, format, dpi=300):
    data = read_data(file)
    image_path = save_wind_rose(data, format=format, dpi=dpi)
    return plot_wind_rose(data), image_path
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(choices=['png', 'jpg', 'svg'], value='png', label="Select File Format")
    ],
    outputs=["html", "file"],
    title="Wind Rose Plot Generator",
    description="Upload a CSV or Excel file with 'date', 'speed', and 'direction' columns to generate a wind rose plot. Choose the file format for saving."
)
